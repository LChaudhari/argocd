...
# Step 4: Add Your Cluster to ArgoCD (if not already added)
...
# 1. Get the Docker IP of your ArgoCD cluster
docker inspect argocd-cluster-control-plane -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}'

# 2. Patch kubeconfig to use that IP instead of 127.0.0.1
kubectl config set-cluster kind-argocd-cluster --server=https://<IP>:6443

# 3. Now add it to ArgoCD
argocd cluster add kind-argocd-cluster --name argocd-cluster --insecure
...
## ###################################################################################
# Once Cluster added to ArgoCD

 Go to http://localhost:8080
 -- Settings >> Cluster >> Verify cluster is available
